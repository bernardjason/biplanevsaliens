...
impl Camera {
    ...
    pub fn LookDownMAtrix(&self) -> Matrix4 {
        Matrix4::look_at(self.Position, self.Position + self.Front + vec3(0.0,-1000.0,0.0), self.Up)
    }
    ...
    pub fn ProcessKeyboard(&mut self, direction: Camera_Movement, delta_time: f32) {
        ...
        if direction == LEFT {
            //let rot: Basis2<f32> = Rotation2::from_angle(Rad(-1.0 * f32::consts::PI/180.0 ));
            //self.Direction = rot.rotate_vector(self.Direction);
            //self.Front.x = self.Front.x + self.Direction.x;
            //self.Front.z = self.Front.z + self.Direction.y;
            //self.Position += -(self.Right * velocity);
            //println!("LEFT {},{}",self.Direction.x,self.Direction.y);
        ...
        if direction == RIGHT {
            ...
            //self.Front.z = self.Front.z + self.Direction.y;
            //self.Position += self.Right * velocity;
            //println!("RIGHT {},{}",self.Direction.x,self.Direction.y);
            self.Yaw=self.Yaw+1.0;
            self.updateCameraVectors();
